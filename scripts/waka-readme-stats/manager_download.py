...
async def init_download_manager(user_login: str):
    ...
    await DownloadManager.load_remote_resources(
        ...
        # 使用 range 参数让 WakaTime 使用用户时区计算日期范围
        waka_summaries=f"https://wakatime.com/api/v1/users/current/summaries?range=Last%207%20Days&api_key={EM.WAKATIME_API_KEY}",
